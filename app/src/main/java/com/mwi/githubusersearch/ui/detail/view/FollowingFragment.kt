...
import kotlinx.android.synthetic.main.fragment_following.lyt_empty_result
...
class FollowingFragment : Fragment() {
    ...
    private fun setupViewModel(username: String) {
        viewModel = ViewModelProviders.of(this, FollowerModelFactory(username, "following"))
    ...
    private fun setupObserver() {
        ...
        viewModel.getNetworkState().observe(this, Observer {
            when (it) {
                Status.LOADING -> {
                    adapter.setState(Status.LOADING)
                }
                Status.SUCCESS -> {
                    adapter.setState(Status.SUCCESS)
                    if (viewModel.listIsEmpty())
                        lyt_empty_result.visibility = View.VISIBLE
                    else
                        lyt_empty_result.visibility = View.GONE
